...
impl ModuleTypesBuilder {
    ...
    ///
    /// The first `EXN_ENGINE_FIELDS` fields of this struct are managed by the
    /// engine and hold the exception's tag reference; the remaining fields are
    /// the exception's payload values, matching the parameters of
    /// `for_func_ty`.
