...
use wasmtime_environ::{
    Collector, EXN_ENGINE_FIELDS, GcArrayLayout, GcLayout, GcStructLayout, GcTypeLayouts,
    I31_DISCRIMINANT, ModuleInternedTypeIndex, TagIndex, TypeIndex, VMGcKind,
...
fn exn_field_vals(
    ...
    let mut vals = SmallVec::with_capacity(payload.len() + EXN_ENGINE_FIELDS);
    vals.push(instance_id);
    vals.push(tag);
    debug_assert_eq!(vals.len(), EXN_ENGINE_FIELDS);
