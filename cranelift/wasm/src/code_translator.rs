...
pub fn translate_operator<FE: FuncEnvironment + ?Sized>(
    ...
    match op {
        ...
        | Operator::V128Store { memarg } => {
            translate_store(memarg, state.pop1(), builder, state, environ)?;
        }
        Operator::I32Store8 { memarg } | Operator::I64Store8 { memarg } => {
            translate_narrow_store(memarg, I8, builder, state, environ)?;
        }
        Operator::I32Store16 { memarg } | Operator::I64Store16 { memarg } => {
            translate_narrow_store(memarg, I16, builder, state, environ)?;
        }
        Operator::I64Store32 { memarg } => {
            translate_narrow_store(memarg, I32, builder, state, environ)?;
        }
        /****************************** Nullary Operators ************************************/
        ...
        | Operator::V128Store64Lane { memarg, lane } => {
            let vector = pop1_with_bitcast(state, type_of(op), builder);
            let val = builder.ins().extractlane(vector, lane.clone());
            translate_store(memarg, val, builder, state, environ)?;
        }
...
fn translate_store<FE: FuncEnvironment + ?Sized>(
    ...
    let mem_op_size = val_ty.bytes() as u8;
...
fn translate_narrow_store<FE: FuncEnvironment + ?Sized>(
    memarg: &MemArg,
    narrow_to: Type,
    builder: &mut FunctionBuilder,
    state: &mut FuncTranslationState,
    ...
    let val = state.pop1();
    let val = builder.ins().ireduce(narrow_to, val);
    translate_store(memarg, val, builder, state, environ)
}
