...
pub fn builder_with_options(
    ...
    // we use getauxval from the auxv crate directly.
    #[cfg(all(target_arch = "s390x", target_os = "linux"))]
    {
        ...
        let q = auxv::getauxval::NativeGetauxval {};
        if let Ok(v) = q.getauxval(auxv::AT_HWCAP) {
            const HWCAP_S390X_VXRS_EXT2: auxv::AuxvType = 32768;
